...
// & | ^ ~ << >> >>>
// &: 2개의 정수를 이진법으로 변환하고 같은 자리가 1일 때만 1이 나오게 된다.
// |: 2개의 정수를 이진법으로 변환하고 같은 자리가 한 개라도 1이면 1이 나오게 된다.
// ^: 2개의 정수를 이진법으로 변환하고 같은 자리가 다를 때만 1이 나오게 된다.
// ~: 1개의 정수를 이진법으로 변환하고 1은 0으로 0은 1으로 변환한다.
// <<: 지정된 숫자만큼 왼쪽으로 비트를 이동한다.
// >>: 지정된 숫자만큼 오른쪽으로 비트를 이동하되 부호는 유지한다.
// >>>: 지정된 숫자만큼 오른쪽으로 비트를 이동하되 무조건 +가 되도록 한ㄷ.
public class Ex03Operator05 {
    public static void main(String[] args) {
        ...
        // 7. >>> 연산
        // 연산자 오른쪽에 있는 자릿수만큼 앞에 0을 붙이고
        // 오른쪽 자리는 오른쪽 숫자만큼 날려준다.
        ...
        // -15가 나온다
        // 왜냐하면, 얘네가 이걸 int로 바꾼 다음에 연산하고
        // 그 결과값을 다시 byte로 바꾸기 때문
        // 즉, 100 1000 앞에 0을 3개 붙이는게 아니라
