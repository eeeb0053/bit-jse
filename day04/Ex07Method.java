 ...
 *  그래서 그렇게 특정 기능들을 다시 사용해야된다면, 즉 그 코드들을 다시 사용해야 한다면
 *  기능으로 분리해서 메소드로 만들면, 우리가 그 메소드를 호출만 해주면
 *  해당 코드들이 불러와지기 때문에
 *  다시 써주지 않아도 된다.
 *  
 *  메소드는 선언과 구현으로 이루어져있다.
 *  선언: 해당 메소드의 이름, 실행할 때 필요로하는 외부의 값, 
 *      메소드가 종료되면서 호출됐던 곳으로 보내줄 때의 데이터타입을 적어주게 된다.
 ...
 *  여러 개의 메소드를 같은 이름으로 묶는 방법을 말한다.
 *  예를 들어, System.out.println()은 총 9가지 파라미터에 대해서
 *  오버로딩이 되어있다.
 *  
 *  예를 들어 우리가 만든 Ex09Carculator 클래스의 경우
 *  add, subtract, multiply, divide 메소드들이
 *  int 파라미터 2개에 대한 선언만 구현만 되어있다.
 *  그렇다면 add(1, 3.1) 혹은 add(3.0, 1.4)같이
 *  더블이 파라미터로 포함되는 경우에는 어떻게 될까?
 *  당연히 에러가 난다. 왜냐하면, 우리가 해당 메소드들에 대해
 *  "int 데이터타입을 가진 값 2개" 에 대해서만 선언과 구현해놨기 때문에
 *  더불어 하나라도 들어오는 순간 해당 파라미터들을 가진 메소드는 존재하지 않기 때문에
 *  에러가 나게 된다.
 *  
 *  따라서 숫자 2개를 더한다 = add 이므로
 ...
 *  -> 답은 4번. 왜냐하면, 실체 우리가 메소드를 호출할 때 중요한 건 파라미터의 이름이 아니라
 *     파라미터의 데이터타입 순서이기 때문이다.
 *     만약 printInfo("서울대")라고 했을 때 1, 2, 3번 중 어떤게 호출되어야 하는가?
 *     정답은 컴퓨터가 정할 수 없다가 된다. 왜냐하면 1, 2, 3번 모두 다 파라미터로 "스트링"이기만 하면 되기때문에
 *     컴퓨터는 "서울대"가 사람 이름인지 학교 이름인지 판단하지 않고
 *     오로지 데이터타입만 생각하기 때문이다.
 *  
 *  따라서 파라미터가 다르다는 것은 메소드의 파라미타의 데이터타입의 순서가 다르다는 뜻이다.
 *  즉 파라미터가 같은지 궁금하면 이름은 무시하고 데이터타입만 쭉 써보면 알 수 있다.
 *  
 *  접근제한자(Access Modifier)
 *  접근제한자란 해당 메소드 혹은 우리가 나중에 배우게 될 필드가
 *  외부 클래스가 사용가능한지를 지정하는 키워드이다.
 *  public, protected, default, private 키워드가 있으며
 *  각각 다음과 같은 범위에서 사용가능하다.
 *  public: 해당 메소드 혹은 필드를 어디서든 접근 가능(외부패키지, 외부 클래스)
 ...
 *           default 키워드는 기본적으로 switch에서 사용되기 때문에
 *           우리가 아무런 접근체한자도 적지않으면 기본적으로 default 접근체한자가 적용된다.
 *           또한, 같은 패키지에서만 접근 가능하기 때문에 "package 접근제한자"라고도 부른다.
 *  private: 해당 메소드 혹은 필드는 모든 외부클래스의 접근이 불가능하다.
 *           오직 해당 클래스 내부에서만 호출 가능하다.
 *           예전에는 클래스 내부에서 숨겨야하는 내용만 private으로 했지만
 *           이제는 외부가 정말 필요한 내용만 public으로 해주고
 *           나머지는 모두 다 private으로 하는게 정석이다.
 *  */
}
