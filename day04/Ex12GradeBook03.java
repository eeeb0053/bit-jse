...
public class Ex12GradeBook03 {
    ...
    static void showMenu(Scanner scanner) {
        ...
        int math = 0;
        while(true) {
            System.out.println("1. 성적 입력 2. 성적 출력 2. 종료");
    ...
    // call by value: 변수를 넘겨주더라도 값만 넘어가고 변수 그 자체는 넘어가지 않음
    // call by reference: 주소값을 넘겨주기 때문에 변경을 가하면 변수도 변경됨
    
    // call by value의 경우, 해당 변수의 "값 복사본"이 넘어가기 때문에
    // 우리가 파라미터의 값을 변화시키더라도 원본 변수의 값은 변하지 않는다.
    
    // call by reference의 경우, 해당 변수의 "주소값"이 넘어가기 때문에
    // 우리가 메소드에서 파라미터를 변화시키면 원본 변수에도 변화가 적용된다.
    
    // call by value는 기본형 데이터타입 변수가 파라미터로 넘어갈 때 발생하고
    // call by reference는 참조형 데이터타입 변수가 파라미터로 넘어갈 때 발생한다.
