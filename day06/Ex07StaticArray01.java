...
// 정적할당의 경우
// 만약 배열의 크기보다 많은 학생을 입력하고자 할 때
// 1. 더이상 입력할 수 없습니다.
// 2. 가장 앞에 있던 애를 지우고 그 다음 인덱스부터 하나씩 당겨서 가장 뒷칸에 넣는 방법
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (userChoice == 1) {
                ...
                // for문을 이용해서 빈칸을 찾아보자
                // 빈칸은 아무런 보가 입력되지 않아있다.
                // 즉, 해당 칸이 초기화되지 않았다는 의미가 된다.
                // 해당 칸이 null인 index를 찾아서 저장하면 된다.

                // null인 index를 찾아주는 for문
                ...
                // 만약 index가 -1이라는 것은
                // 모든 칸이 null이 아니다라는 뜻
                // 즉 모든 칸에 학생의 정보가 존재하므로 메세지만 출력하고
                // 그 외에만 studentArray[index]에
                // 정보를 입력하면 된다.
                if (index == -1) {
                    // 배열의 모든 칸에 정보가 있으므로 메시지 출력
