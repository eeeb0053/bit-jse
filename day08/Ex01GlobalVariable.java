...
public class Ex01GlobalVariable {
    ...
    // 하지만 3세대 언어에선 이 위치에 우리가 변수를 선언하면?
    // 변수가 아닌 필드가 된다.
    
    // 그렇다면 모든 메소드가 쓸 몇개의 변수를
    // private 선언하고, 겟터/셋터를 만들지 않으면?
    // 조금이나마 해당 변수의 무결성을 올려줄 수 있다.
    
    // 필드와 전역변수의 차이점
    // 필드: 객체를 만들고나서 필드의 값을 정해줌 
    //     (겟터/셋터를 사용하거나 생성자를 사용하거나)
    //     따라서 필드 선언에는 초기화식이 없음.
    // 전역변수: 선언과 초기화가 이루어져있어서 객체마다 해당 변수의 값이 같음   
}
