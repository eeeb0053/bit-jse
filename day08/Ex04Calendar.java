...
// 즉 구식이 되었으므로
// 시간을 표시하거나 설정할 때는 Calendar를
// 시간의 차이를 계산할 때에는 Chronicle을 사용하는 것을 권장하고 있다.
...
// 1970년 1월 1일부터 지금까지를
// 1/1000초 단위로 계산한다.
// 그것을 기반으로 여러가지 값으로 보여주거나
// 혹은 우리가 DataFormat이란 것을 사용하여
...
public class Ex04Calendar {
    public static void main(String[] args) {
        ...
        // 시간을 예쁘게 출력해보기
        // 시간을 예쁘게 출력할 때에는 DateFormat 혹은 SimpleDateFormat 클래스 객체를 통해서
        // 우리가 포맷을 정해주어야 한다.
        // 포맷의 경우
        // 년: y  월: M  일: d  
        // 시: 12시간은 h 24시간은 H  분: m  초: s
        // 를 사용하여 직접 지정해준다.
        
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 dd일 HH시 mm분 ss초");
        
        // DateFormat의 경우 1/1000초 단위를 지정한 양식으로 예쁘게 String으로 바꿔주는 역할도 하지만
        // 입력한 String이 지정한 양식과 일치하면 1/1000초로 바꾸는 역할도 한다.
        ...
        // 만들어진 String을 sdf의 parse를 이용해서 캘린더로 바꿔보자
        // 단 시간이 잘못됐거나 바꿀 수 없는 경우가 존재하기 때문에
        // try catch라는 것을 통해 감싸주어야 한다.
        
        // try/catch 구조란
        // 혹시라도 발생할 수 있는 심각한 오류에 대비하여
        // 오류가 날 수 있는 코드를 try에서 실행하고
        // 오류가 발생한다면 catch가 해결하는 형태의 코드이다.
        
        try {
            // sdf가 변환한 temp를 cal에 저장
            // 만약 사용자가 정확하지 않은 값을 입력했을 때
            // cal 객체의 세팅에 따라 결과가 나뉜다.
            // 1. lenient 모드 (자비로운 모드)
            //    만약 잘못된 시간형식이면
            //    유도리있게 시간을 환산해준다.
            //    예를 들어 월을 25월로 입력한다면 알아서 2년 1개월로 환산하여
            //    년에 2년을 더하고 1월로 설정한다.
